...
class Runner(object):
    ...
    def runas(self, cmd):
        ...
        parameters = (
            '-c "import subprocess; ' "subprocess.run('{}', shell=True)" '"'
        ).format(cmd)
...
def parse_args(tasks={}):
    ...
    parser.add_argument(
        "-a", "--all", help="Run all available tasks", action="store_true"
    )
    parser.add_argument(
        "command",
        nargs="*",
        # choices=tasks.keys(),
        # Causes error in combination with nargs='*',
    ...
    if args.all and args.command:
        print(
            'Warning: if the "--all" option is given, '
            "no commands need to be specified.\n"
        )
    if args.all:
        return tasks.keys()
    if args.command:
...
def main():
    ...
    logging.info("Running tasks from script: {}".format(sys.argv[0]))
    tasks = collect_tasks()
